...
from src.iot_manager import ActuatorCommand, IoTNodeRegister, TelemetryPayload, iot_manager
...
class CoordinateQuery(BaseModel):
    latitude: float = Field(
        ...
        ge=-4.5,
        le=13.5,
        description="Latitud en Venezuela (WGS84)",
        json_schema_extra={"example": 9.324},
    )
    longitude: float = Field(
        ...,
        ge=-73.5,
        le=-59.5,
        description="Longitud en Venezuela (WGS84)",
        json_schema_extra={"example": -69.112},
    )
    ph: Optional[float] = Field(6.2, ge=3.5, le=9.5, description="pH del suelo (opcional)")
    organic_matter_pct: Optional[float] = Field(
        3.0, ge=0.2, le=12.0, description="Materia Orgánica % (opcional)"
    )
    texture: Optional[str] = Field("Franco-limoso", description="Textura del suelo (opcional)")
    parcel_area_ha: Optional[float] = Field(
        10.0,
        ge=0.1,
        le=10000.0,
        description="Área en hectáreas del polígono (calculado vía Shoelace geodésico)",
    )
    force_refresh: bool = Field(
        False, description="Forzar consulta satelital en vivo omitiendo caché"
    )


class ChatConsultQuery(BaseModel):
    latitude: float = Field(
        ..., ge=-4.5, le=13.5, description="Latitud (WGS84) para proveer contexto espacial a Gemini"
    )
    longitude: float = Field(
        ...,
        ge=-73.5,
        le=-59.5,
        description="Longitud (WGS84) para proveer contexto espacial a Gemini",
    )
    message: str = Field(..., min_length=1, description="Pregunta o consulta del productor")
    conversation_history: Optional[List[Dict[str, str]]] = Field(
        default=[], description="Historial de mensajes previos"
    )
...
@app.post(
    "/api/v1/spatial/profile", response_model=SpatialProfileResponse, tags=["Perfil Espacial"]
)
...
def predict_risks_and_carbon(query: CoordinateQuery):
    ...
    carbon = risk_carbon_engine.model_carbon_sequestration(
        feat_result["features_dict"], query.parcel_area_ha or 10.0
    )
...
def generate_ai_prescription(query: CoordinateQuery):
    ...
    return {
        ...
        "key_metrics": {
            ...
            "ndvi_vigor": spatial_profile.sentinel_vegetation.get("latest_metrics", {}).get(
                "vegetation_vigor"
            ),
        },
    }
...
def consult_gemini_advisor(query: ChatConsultQuery):
    ...
    spatial_data = get_unified_spatial_profile(
        CoordinateQuery(latitude=query.latitude, longitude=query.longitude)
    )
    parcel_context = {
        "coordinates": {"latitude": query.latitude, "longitude": query.longitude},
...
def evaluate_mapbiomas_discrepancy(query: CoordinateQuery):
    ...
    """
    spatial_profile = get_unified_spatial_profile(query)
    mapbiomas_class_id = spatial_profile.mapbiomas_lulc.get("latest_coverage_2024", {}).get(
        "class_id", 18
    )
...
def ingest_iot_telemetry(
    ...
    x_device_token: str = Query(
        "sec_iot_node_turen_001", description="Token de autenticación del dispositivo"
    ),
    forecast_rain_6h_mm: float = Query(
        0.0, description="Lluvia pronosticada (NASA POWER) en 6 horas"
    ),
):
    """
    ...
    try:
        return iot_manager.process_telemetry(
            payload=payload, token=x_device_token, forecast_rain_6h_mm=forecast_rain_6h_mm
        )
