...
class MapBiomasYieldOptimizer:
    ...
    def generate_yield_optimization_strategy(self, lat: float, lng: float, crop: str, current_ph: float) -> Dict[str, Any]:
        """
        ...
        if "Bosque" in historical_cover:
            ...
            if current_ph < 5.5:
                recommendations.append("Encalado preventivo (Roca Fosfórica) para evitar toxicidad por Aluminio.")
            yield_potential_modifier = 1.15
            
        elif "Pastura" in historical_cover:
            recommendations.append("Realizar labranza profunda (arado de cincel) para romper la capa compactada por pisoteo bovino.")
            ...
            recommendations.append("Incorporar abonos verdes o rastrojo para elevar el Carbono Orgánico (SOC).")
            yield_potential_modifier = 0.95
            
        elif "Continua" in historical_cover:
            recommendations.append("Rotación de cultivos urgente (ideal leguminosas) para romper ciclos de plagas.")
        ...
        if climate_data["drought_stress_index"] > 0.6:
            recommendations.append("Ajustar densidad de siembra un 10% a la baja para mitigar estrés hídrico (Data NASA POWER).")
