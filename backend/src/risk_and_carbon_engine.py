...
class RiskAndCarbonEngine:
    ...
    def evaluate_risks(self, feature_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if ph < 5.0:
            ...
            acidity_score = 90
            acidity_action = "Encalado urgente con Cal Dolomítica (CaCO3 + MgCO3) para desbloquear Fósforo."
        elif ph < 5.8:
        ...
        if temp > 30.0:
            thermal_level = "ALTO"
            thermal_score = 72
            thermal_action = "Seleccionar variedades tolerantes a altas temperaturas y sombrío temporal."
        elif temp > 27.5:
        ...
        overall_risk_index = round((drought_score + flood_score + acidity_score + thermal_score) / 4.0, 1)

        return {
            "overall_risk_index_pct": overall_risk_index,
            "overall_risk_category": "ALTO" if overall_risk_index > 60 else "MODERADO" if overall_risk_index > 35 else "BAJO",
            "risk_breakdown": {
                "drought_stress": {"level": drought_level, "score": drought_score, "mitigation": drought_action},
                "waterlogging_flood": {"level": flood_level, "score": flood_score, "mitigation": flood_action},
                "soil_acidity_aluminum": {"level": acidity_level, "score": acidity_score, "mitigation": acidity_action},
                "thermal_extreme": {"level": thermal_level, "score": thermal_score, "mitigation": thermal_action}
            }
        }

    def model_carbon_sequestration(self, feature_dict: Dict[str, Any], parcel_area_ha: float = 10.0) -> Dict[str, Any]:
        """
